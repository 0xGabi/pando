...
function File(props) {
  return (
    <Highlight {...defaultProps} code={props.file} language="javascript" theme={undefined}>
...
function Folder(props) {
  const entries = props.tree.map(entry => <Entry key={entry.name} entry={entry} forward={props.forward} display={props.display} />)
  ...
  render() {
    ...
    const content = file ? (
      ...
      <Container>
        <Table>
          <tbody>
            <Folder tree={tree} parents={parents} forward={this.forward} backward={this.backward} display={this.display} />
          </tbody>
        </Table>
      </Container>
    )
